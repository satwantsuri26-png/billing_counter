...
while serving:
    ...
    amount = int(input(f"Hello {name}! Enter the amount of items you have bought':"))
    if amount <=0:
        print ("The amount of items is invalid, please enter it again: ")
        continue

    print (f"/n Dispersing {amount} units for {name}: ")
    remaining = amount
    idx = 1
    while idx <=6:
       if idx == 1: value = 100
    ...
    for slot in range (1,7):
        ...
        if total > 0 :
            print (f"{value} - unit note(s) dispensed: {total}", ends= "")
            for note in range (total):
                print ("=")
                print ()
print (f"/n customers serverd :  {customers_served}")
print (f"/n total dispensed : {total_dispensed} units")
